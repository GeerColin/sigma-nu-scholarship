...
- [x] Refined diagnostic deployment `1e94710` is Ready on the stable alias. Three desktop and three mobile seven-route rounds plus seven after-login fresh loads passed (49 authenticated attempts). Logout blocked all seven protected routes; real Google login restored the session. Including clean-session and post-restart testing, overall explicit counts are 94 authenticated fresh attempts, 90 passed/four failed, plus seven correctly denied signed-out requests (101 total). Automatic OAuth redirects/prefetches are excluded.
...
- [~] Further primary-source research found a confirmed hosting-locality mismatch: deployment Resources shows Node.js functions in `IAD1`, while Supabase is Oregon (`us-west-2`). Added a single-region Vercel configuration for `pdx1`, with a unit regression. This follows Vercel's co-location guidance without a paid upgrade, database move, deprecated Next.js region export, or retry/RLS/visual change. The failed invocation's instance age was 219,837 ms and execution 6.48 seconds against a five-minute maximum; no execution-limit fix is indicated. Region deployment and follow-up stress results remain to be verified; this is a latency mitigation, not a proven provider root-cause fix.
